...
> 第一次使用？从 [Project Lens 使用教程](./USAGE.md) 开始，了解界面、任务格式和推荐项目文档。

## 它解决什么问题

- **AI 改得太快，人来不及看。** 项目方向、当前步骤和修改原因经常散落在长对话里，下一次继续工作时很难接上。
- **文档很多，却不知道下一步看哪里。** 需求、设计、技术、测试和发布说明彼此分散，Project Lens 用总览连接它们，并从任务直接定位到真实 Markdown。
- **已经确认的功能可能被后续修改破坏。** 用户确认后的功能和设计会成为受保护基线；AI 再次修改前必须说明范围、文件与风险。
- **只看到代码变化，却不知道它对应什么功能。** 本轮修改、历史记录和 `CHANGELOG.md` 分别说明正在改什么、哪些内容已确认、正式版本改变了什么。
- **项目密钥的用途不透明。** AI 只能申请密钥名称、用途和精确写入位置，密钥值由 Project Lens 在用户批准后代写给当前项目。

## 适合谁使用

- 主要通过 Vibe Coding 把想法做成产品，希望把注意力放在方向、功能和结果上的创作者。
- 使用 AI 编码，但不想在长对话里失去项目方向的开发者。
- 不熟悉全部代码，却需要判断功能是否真正完成的产品负责人或独立开发者。
- 同时使用多个 AI 工具，希望它们维护同一套项目事实和文档的人。
- 希望保留本地 Markdown、Git 和真实文件，不想把项目状态锁在云端工具中的团队。
...
![用户核对用途与精确写入位置后批准密钥申请](assets/project-lens-key-approval.png)
...
支持方式将在这里更新。Project Lens 的应用内入口会直接定位到本章节，不会在应用内处理付款。
...
把 Project Lens 拖入“应用程序”后，第一次双击可能看到“无法打开 Project Lens”或“Apple 无法验证其中是否包含恶意软件”。这是未公证预览版的首次安全提示，不表示需要关闭 macOS 的安全保护。

1. 在“应用程序”中双击 Project Lens 一次，让安全提示出现，然后关闭提示。
2. 打开“系统设置”，选择“隐私与安全性”。
3. 向下找到“安全性”区域，在 Project Lens 的拦截说明旁点击“仍要打开”。
4. 在最后一次确认中选择“打开”。以后可正常启动。

![macOS 首次阻止 Project Lens 时允许打开的位置](assets/project-lens-open-anyway.png)

如果没有看到“仍要打开”，请先回到“应用程序”再尝试启动 Project Lens 一次，然后重新打开“隐私与安全性”。不需要执行终端命令，也不要关闭 Gatekeeper。

如果提示安装包“已损坏，无法打开”，请不要继续授权；重新下载与 Mac 架构匹配的 DMG，并使用 `SHA256SUMS.txt` 检查下载是否完整。

## 开始使用

1. 下载并把 Project Lens 拖入“应用程序”。
2. 第一次启动时，按上面的图示允许 macOS 打开应用。
3. 在应用中选择一个真实项目文件夹。
