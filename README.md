...
总览把项目工作按“未开始 / 进行中 / 待验收 / 已完成”排成四列看板。功能、阶段和任务各自是一张卡片；有下级内容的卡片会进入新的子看板，顶部面包屑始终保留所在层级，因此复杂规划也不需要挤在一张长页面里。

点击末级任务会立即显示对应 Markdown 原文预览；选择“打开文档”后，Project Lens 在下方文档栏打开真实文件，并定位到对应标题或步骤。文档栏可以随时关闭，项目总览不会因此改变。
...
用户先沿看板掌握方向，再按需查看细节；AI 也能沿着同一条链接找到应该维护的位置。
