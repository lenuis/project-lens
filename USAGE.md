...
4. 如果项目已经进行到中途但文档结构不同，Project Lens 会先显示从真实 Markdown 派生的只读总览；你可以在总览下方选择实际 AI 工具并发起“重新检查并整理”。
5. 项目、所选工具和未完成整理状态都会保留，下次打开应用可以继续使用。
...
- Project Lens 不生成需要维护的 HTML 总览；日常使用始终从真实 Markdown 生成动态视图。

![Project Lens 动态总览](./assets/project-lens-demo-overview.png)

## 2. 看懂主界面

- **动态总览**：用“未开始 / 进行中 / 待验收 / 已完成”四列看板显示项目方向与进度；点击有子任务的卡片进入下一级，使用面包屑返回。
- **左侧文档列表**：项目中真实存在的 Markdown 文件。点击后会在下方文档区打开。
- **下方 Markdown 文档区**：阅读或编辑当前文档；末级任务先显示即时原文预览，选择“打开文档”后自动定位并高亮对应内容。文档栏不需要时可以整体关闭。
...
点击有子任务的卡片会进入新的四列子看板；点击末级任务会先显示原文预览，再按需打开关联文档并定位到对应内容，而不是把整份详情重复塞进总览。
...
功能说明、设计要求、技术章节，以及不用复选框表达的步骤，可以把状态写在二至四级标题的下一行：
...
Project Lens 会显示“未实现”“实现中”或“已实现 - 本地时间”。只有真实实现并验证后才写完成时间；旧章节没有可靠时间时不要补造。复选框任务仍使用 `[ ] / [~] / [x]` 和任务自己的完成时间，不重复添加章节状态。
...
1. 在侧边栏总览下选择你实际使用的 AI 工具；Project Lens 不会根据进程名猜测。
2. 点击“重新检查并整理”。Project Lens 只准备规则、整理请求和完整指令，不会在后台静默发送任务。
3. 复制指令，在已经打开该项目的 AI 对话中发送。
4. AI 重新读取现有 Markdown，分类需求、设计、技术、测试和发布说明，并把主方向整理到 `PROJECT.md`。
5. 复制后交接区会收起；所选工具、整理请求与完成状态按项目保存，切换项目或重启后仍可继续。
